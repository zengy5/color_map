...
if __name__ == "__main__":
    ...
    # same_value_row = {}
    # for i, row in enumerate(point_pixel_coords):
    #     row_tuple = tuple(row[0])
    #     if row_tuple in same_value_row:
    #         same_value_row[row_tuple].append(i)
    #     else:
    #         same_value_row[row_tuple] = [i]
    # duplicate_rows = [indices for indices in same_value_row.values() if len(indices) > 1]
    # print("具有相同值的行的索引：", duplicate_rows)
    ...
    for point in point_pixel_coords:
        x = point[0][0]
        y = point[0][1]
        if 360<=y<=400 and 620<= x <= 660:
            point_colors_bgr[i] = [1,1,0]
        i += 1
    ...
    image = cv2.imread(bef_pic_name) # load the image
    points = np.asarray(pcd.points)
    num_points = points.shape[0]
    point_matrix = np.ones((num_points, 4))
    point_matrix[:, :3] = points  
    ...
    for point in point_pixel_coords:
        x = point[0][0]
        y = point[0][1]
        if 0 <= y < result_image.shape[0] and 0 <= x < result_image.shape[1] and point_matrix[i,0]>=0 and point_matrix[i,0]<=5.0:
